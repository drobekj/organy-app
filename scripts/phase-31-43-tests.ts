...
async function main(): Promise<void> {
  ...
  try {
    const { contract, handoffText } = await readDefinitiveArchive(archivePath!);
    ...
    const rawA = unzipJson<Record<string, unknown>>("A-melody-equivalence.json");
    const rawB = unzipJson<Record<string, unknown>>("B-jaroslav-repertoire-pivots.json");
    const rawC = unzipJson<Record<string, unknown>>("C-validation-report.json");
    const embeddedHandoff = spawnSync("unzip", ["-p", archivePath!, "HANDOFF.md"], { encoding: "utf8" }).stdout;
