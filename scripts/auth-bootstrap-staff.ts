...
async function main() {
  ...
  if (!actorUserId || !displayName || !username || !role || !["admin", "priest", "organist"].includes(role)) {
    throw new Error("Required: --actor-id ID --display-name NAME --role admin|priest|organist --username USERNAME [--person-id ID].");
  }
  ...
  try {
    if (personId) {
      const person = await pool.query("select 1 from catalog_persons where id = $1", [personId]);
      if (person.rows.length === 0) throw new Error("person-id does not exist in catalog_persons.");
    }
    await pool.query(
      "insert into app_users (id, display_name, person_id, active) values ($1,$2,$3,true) on conflict (id) do update set display_name = excluded.display_name, person_id = coalesce(excluded.person_id, app_users.person_id), active = true",
      [actorUserId, displayName, personId ?? null],
    );
