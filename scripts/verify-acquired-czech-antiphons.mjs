...
function expectedPublicUrl(record) {
  ...
  if (publicUrl) {
    try {
      ...
      // Replay the extractor's structured-route fallback.
    }
  }
  const publicRoute = String(record.song_lyric?.public_route ?? "").trim();
  if (!publicRoute) throw new Error(`Replay source has no acceptable URL or route at ${record.number}.`);
  const resolved = new URL(publicRoute, "https://www.evangelickykancional.cz");
  if (resolved.protocol !== "https:" || resolved.origin !== "https://www.evangelickykancional.cz") throw new Error(`Replay source has wrong route origin at ${record.number}.`);
...
for (let index = 0; index < records.length; index++) {
  ...
  if (records[index].url !== expectedPublicUrl(source)) throw new Error(`Structured URL precedence mismatch at antiphon ${records[index].number}.`);
}
