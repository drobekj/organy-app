...
assert.match(candidateListSource, /function returnToVisibleCandidate\(candidate: CandidateQueryResult, index: number\)[\s\S]*?setActiveIndex\(index\)[\s\S]*?setDetailReturnSongId\(candidate\.songId\)[\s\S]*?setDetailReturnCandidates\(\[\.\.\.rawVisibleCandidates\]\)[\s\S]*?props\.onOpen\(\)/, "clicking the exposed list under candidate Detail closes Detail back to that exact visible candidate without selecting it");
...
assert.match(candidateListSource, /if \(candidateDetailOpen\) \{[\s\S]*?querySelector<HTMLElement>\("\.melody-detail-candidate"\)[\s\S]*?listRef\.current\?\.contains\(target\)[\s\S]*?target\.closest<HTMLElement>\('\[id\^="selected-song-detail-button-"\]'\)[\s\S]*?props\.onBackFromDetail\(\)[\s\S]*?return;[\s\S]*?pendingFullCandidateDismiss\.current = true;[\s\S]*?setSuppressCandidateOverlay\(true\)/, "the selected-song Detail button is an explicit switch exception before the generic candidate-origin outside-dismiss path");
...
assert.doesNotMatch(candidateListSource, /suppressOutsideDetailClick|suppressCandidateDismissClick/, "the selected-song Detail switch must not be swallowed by candidate-origin click suppression");
...
assert.match(candidateListSource, /\.row-icon-palette \.row-icon-button:not\(\.row-icon-remove\) \{[\s\S]*?-webkit-text-stroke: 0\.35px currentColor;[\s\S]*?text-shadow:/, "arrow glyphs receive additional stroke weight without changing control geometry");
assert.match(candidateListSource, /className=\{`candidate-option-row \$\{viewModel\.backgroundClass\}\$\{current \? " candidate-option-current" : ""\}\$\{index === activeIndex && !current \? " candidate-option-active" : ""\}`\}/, "candidate cursor state belongs to the whole candidate row including Detail");
