...
const LOG_PATH = resolve(process.cwd(), "phase-31-12-human.log");
const PASS = "Phase 31.12 authoritative Planning candidates: HUMAN PASS";
const FAIL = "Phase 31.12 authoritative Planning candidates: HUMAN FAIL";
...
async function seedCheckpointCandidates(databaseUrl: string) {
  const pool = new Pool({ connectionString: databaseUrl, max: 1 });
  try {
    const profiles = (await pool.query("select id,category from preference_profiles order by category,id")).rows;
    if (!profiles.length) throw new Error("Checkpoint database is missing preference profiles.");
    ...
    await log("Seeded Phase 31.12 authority: antiphon czech:800 -> song czech:1; czech:1 in demo-organist repertoire; czech:1/polish:1 melody equivalents.");
  } finally { await pool.end(); }
}
...
async function main() {
  await writeFile(LOG_PATH, "Phase 31.12 local browser checkpoint\n", "utf8");
  ...
  try {
    ...
    await seedCheckpointCandidates(databaseUrl);
