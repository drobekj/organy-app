...
assert.doesNotMatch(guideControls, /"development\.runtime"\s*:/, "The new panel summary map must not add Development.");
...
const addRow = planning.indexOf('data-guide-hint="planning.rows.add"', rowsPanelStart);
assert.ok(rowsListStart >= 0 && addRow > rowsListStart, "Add row must remain below the rows list.");

assert.match(protection, /data-guide-hint-scope="planning\.melody-protection"/);
assert.match(protection, /data-guide-hint="planning\.melody-protection"/);
