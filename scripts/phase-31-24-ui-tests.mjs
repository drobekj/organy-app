...
assert.match(client, /selectedRole === "priest" \|\| selectedRole === "organist" \|\| selectedRole === "admin"[\s\S]*?<NonRepetitionPeriodPanel[\s\S]*?runtimeMode=\{runtimeMode\}[\s\S]*?actor=\{activeActor\}/, "Planning must wire Melody Protection for Priest, Organist, and Admin");
assert.match(client, /selectedOrganistPersonId=\{organistId\}/);
assert.match(client, /effectiveMonths=\{selectedRole === "admin" \? adminCandidateMelodyProtectionMonths : melodyProtectionMonths\}/);
...
assert.match(panel, /if \(actor\.role !== "priest" && actor\.role !== "organist" && actor\.role !== "admin"\) return null;/, "Priest, Organist, and Admin receive the Planning Melody Protection control");
...
assert.match(panel, /getOrganistMelodyProtection/, "Priest and Admin must read the selected Organist minimum");
assert.match(panel, /actor\.role === "admin"[\s\S]*?onEffectiveChange\(months\)[\s\S]*?return;/, "Admin changes must stay local to Planning instead of calling a persistence API");
