...
assert.match(
  css,
  /\.workspace-account-popover a,[\s\S]*?\.workspace-account-popover button:not\(\.workspace-toggle-switch\) \{[\s\S]*?text-align: left;[\s\S]*?width: 100%;/,
  "Ordinary popover actions keep their established width and left-aligned labels while compact switches are excluded",
);
assert.doesNotMatch(
  css,
  /\.workspace-account-popover button \{[\s\S]*?width: 100%;/,
  "Compact nested controls must not inherit the generic full-width popover button geometry",
);
