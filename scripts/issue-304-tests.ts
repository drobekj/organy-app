...
assert.match(account, /className="workspace-sign-role-arrow"[^>]*>▸<\/span>/, "Sign Role menu must expose an arrow");
...
assert.match(css, /\.workspace-sign-role-menu\[open\] > summary \.workspace-sign-role-arrow \{[\s\S]*?transform: rotate\(90deg\);/, "Sign Role arrow must indicate expanded state");
