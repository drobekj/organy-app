...
const run = ({ command, args }: E1ChildProcessInvocation, env = process.env) => new Promise<number>((resolve, reject) => {
  const child = spawn(command, args, { env, stdio: "inherit" });
  child.on("error", reject); child.on("close", (code) => resolve(code ?? 1));
});

async function main(): Promise<void> {
  const composeCode = await run({ command: resolveDockerExecutable(), args: ["compose", "up", "-d", "postgres"] });
  ...
  const verification = createNpmInvocation(process.execPath, process.env.npm_execpath, ["run", "verify:engineering-e1"]);
  process.exitCode = await run(verification, { ...process.env, DATABASE_URL: LOCAL_URL });
}
