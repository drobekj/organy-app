...
const LOG_PATH = "phase-31-15-human.log";
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const invocation = createNpmInvocation(process.execPath, process.env.npm_execpath, ["run", "verify:phase-31-15"]);
    await new Promise<void>((resolve, reject) => {
      ...
      child.on("close", async (code) => {
        ...
        code === 0 ? resolve() : reject(new Error(`verify:phase-31-15 exited with code ${code ?? 1}.`));
      });
    });
    console.log("Phase 31.15 current-service melody occupancy and collision validation: HUMAN PASS");
...
void main().catch((error: unknown) => {
  console.error("Phase 31.15 current-service melody occupancy and collision validation: HUMAN FAIL");
  console.error(error);
