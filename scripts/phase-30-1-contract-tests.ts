...
const deniedKnowledge = serviceActor.success ? await interactionService.setMelodyWindow(serviceActor.value, { months: 1 }) : serviceActor;
assert.equal(deniedKnowledge.success, false);
const adminActor = await interactionService.resolveActor("demo-admin-user", "admin");
const serviceWindow = adminActor.success ? await interactionService.setMelodyWindow(adminActor.value, { months: 2 }) : adminActor;
assert.equal(serviceWindow.success && serviceWindow.value.months, 2);
