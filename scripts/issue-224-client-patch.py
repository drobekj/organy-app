...
path = Path("app/planning-lifecycle-client.tsx")
text = path.read_text()
...
replace_once(
  ...
  const lifecycleState = completedRecord ? "completed" : persistedSet?.status ?? "working draft";''',
    "derived conflict/history state",
)
marker = '''  ].filter((message, index, messages) => messages.indexOf(message) === index);

  useEffect(() => {
    const request = ++selectedCandidateAvailabilityRequest.current;'''
replacement = '''  ].filter((message, index, messages) => messages.indexOf(message) === index);
  ...
  useEffect(() => {
    const request = ++selectedCandidateAvailabilityRequest.current;'''
replace_once(marker, replacement, "completed preflight effect")

start = text.index("  async function saveCompletedChanges() {")
end = text.index("\n\n  async function deleteCompletedRecord()", start)
old_save = text[start:end]
new_save = '''  async function saveCompletedChanges() {
...
replace_once(
    ...
    '''          <section className="db-workspace" aria-label="Plans">
            ...
            <div className="rows-header"><h2>Working plans</h2><button type="button" onClick={startNewDbDraft}>Start new set</button></div>''',
    "plans revision alert",
)
replace_once(
    '{completedRecords.length === 0 ? <p className="field-help">No completed service records saved yet.</p> : <ul className="saved-set-list">{completedRecords.map((record) =>',
    '{completedRecordsNewestFirst.length === 0 ? <p className="field-help">No completed service records saved yet.</p> : <ul className="saved-set-list history-scroll-list">{completedRecordsNewestFirst.map((record) =>',
    "history order and scroll class",
)
replace_once(
    '''            {rows.map((row, index) => {
              return (
    ...
    '''            {rows.map((row, index) => {
              ...
              return (
                <fieldset className={`row-card${revisionConflict ? " needs-revision-row" : ""}`} key={row.id}''',
    "conflicting row class",
)
replace_once(
...
css_marker = '''.needs-revision-message {
  color: var(--danger);
  font-size: 0.875rem;
  font-weight: 700;
...
if css.count(css_marker) != 1:
    raise SystemExit(f"CSS marker: expected exactly 1 match, found {css.count(css_marker)}")
...
'''
css_path.write_text(css.replace(css_marker, css_extra, 1))
