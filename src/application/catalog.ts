...
import { decodeCatalogNumberForDisplay, normalizeCatalogSearchQuery } from "./catalog-number";
...
export type CatalogSong = { songId: string; language: ConcreteSongLanguage; number: string; title: string; active: boolean; sheetMusicUrl?: string; sourceUrl?: string };
export type CatalogSongImportRecord = { language: unknown; number: unknown; title: unknown; active?: unknown; sheetMusicUrl?: unknown; sourceUrl?: unknown };
...
export class DrizzleCatalogRepository implements CatalogRepository {
  ...
  async searchSongs(languages: ConcreteSongLanguage[], query: string) { const terms = normalizeCatalogSearchQuery(query); const predicates = terms.map((term) => or(ilike(catalogSongs.number, `%${term}%`), ilike(catalogSongs.title, `%${term}%`))); const rows = await this.db.select().from(catalogSongs).where(and(eq(catalogSongs.active, true), inArray(catalogSongs.language, languages), or(...predicates))).orderBy(asc(catalogSongs.language), sql`${catalogSongs.number}::int`); return rows.map(mapSong); }
  async listSongs() { return (await this.db.select().from(catalogSongs).orderBy(asc(catalogSongs.language), sql`${catalogSongs.number}::int`)).map(mapSong); }
  async upsertSong(song: CatalogSong) { const now = new Date(); const [r] = await this.db.insert(catalogSongs).values({ ...song, sheetMusicUrl: song.sheetMusicUrl ?? null, sourceUrl: song.sourceUrl ?? null, createdAt: now, updatedAt: now }).onConflictDoUpdate({ target: catalogSongs.songId, set: { title: song.title, active: song.active, sheetMusicUrl: song.sheetMusicUrl ?? null, sourceUrl: song.sourceUrl ?? null, updatedAt: now } }).returning(); return mapSong(r); }
...
export async function getEligiblePersonDefaultById(repo: Pick<CatalogRepository, "findPersonById">, id: string | undefined, role: PersonRole): Promise<{ id: string; displayName: string } | undefined> {
...
function mapSong(r: CatalogSongRecord): CatalogSong { return { songId: r.songId, language: r.language, number: decodeCatalogNumberForDisplay(r.number), title: r.title, active: r.active, ...(r.sheetMusicUrl ? { sheetMusicUrl: r.sheetMusicUrl } : {}), ...(r.sourceUrl ? { sourceUrl: r.sourceUrl } : {}) }; }
