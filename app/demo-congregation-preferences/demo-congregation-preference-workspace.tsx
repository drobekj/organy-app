...
export function DemoCongregationPreferenceWorkspace({
  records,
  preferences,
}: {
  ...
  function handleSearchKeyDown(event: KeyboardEvent<HTMLInputElement>) {
    if (event.key === "ArrowDown") {
      ...
      if (firstRecord) {
        ...
        requestAnimationFrame(() => scrollRecordIntoView(firstRecord.id));
      }
  ...
  function togglePreference(referenceSongId: string) {
    setScores((current) => ({
      ...current,
      [referenceSongId]: (current[referenceSongId] ?? 0) === 1 ? 0 : 1,
    }));
  }
  ...
  return (
    <section className="congregation-preference-workspace" aria-label="Reference congregation song preferences">
      ...
      <fieldset className="field-group congregation-find-song-panel" aria-label="Find Song">
        ...
        <div
          ...
          role="list"
          aria-label={language + " reference congregation songs"}
        >
            ...
            return (
              ...
              >
                ...
                <button
                  ...
                  aria-label={(selected ? "Remove" : "Add") + " reference preference for " + record.displayNumber + " " + record.title}
                  onClick={() => togglePreference(record.id)}
                >
...
function rowDomId(referenceSongId: string): string {
  return "demo-congregation-song-" + referenceSongId.replace(/[^a-zA-Z0-9_-]/g, "-");
}
