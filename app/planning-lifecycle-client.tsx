...
export class DbInteractionClient implements InteractionClient {
  ...
  async getReferenceAntiphonRecommendation(input: { actor: ActorIdentity; antiphonId: string }) { return this.transport("getReferenceAntiphonRecommendation", { antiphonId: input.antiphonId }, input.actor); }
...
export class MemoryInteractionClient implements InteractionClient {
  ...
  async getReferenceAntiphonRecommendation() { return { success: false as const, error: { code: "permissionDenied" as const, message: "Reference recommendations are available only in DB runtime." } }; }
...
export default function PlanningLifecycleClient({ runtimeMode }: PlanningLifecycleClientProps) {
  ...
  const [referenceRecommendation, setReferenceRecommendation] = useState<ReferenceAntiphonRecommendation | null>(null);
  ...
  useEffect(() => {
    ...
    void interactionClient.getReferenceAntiphonRecommendation({ actor: activeActor, antiphonId: selectedReferenceAntiphonId }).then((result) => {
      if (result.success) recommendationRequests.current.complete(token, result.value, setReferenceRecommendation);
    });
  }, [runtimeMode, selectedReferenceAntiphonId, activeActor.userId, activeActor.role, interactionClient]);

  useEffect(() => {
    const query=recommendationSongSearch.trim(); const token=recommendationSearchRequests.current.begin(); setRecommendationSongResults([]);
    if(runtimeMode!=="db"||selectedRole!=="admin"||!selectedReferenceAntiphonId||!query)return;
    void referenceClient.list({search:query,pageSize:50}).then((page)=>{if(recommendationSearchRequests.current.isCurrent(token))setRecommendationSongResults(page.records);});
  },[runtimeMode,selectedRole,selectedReferenceAntiphonId,recommendationSongSearch,referenceClient]);
