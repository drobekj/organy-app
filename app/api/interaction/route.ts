...
const pgCatalog = { listSongs: async () => {
  ...
  return rows.map((row) => ({ songId: String(row.song_id), language: row.language as "czech" | "polish", number: decodeCatalogNumberForDisplay(String(row.number)), title: String(row.title), active: Boolean(row.active), ...(row.sheet_music_url ? { sheetMusicUrl: String(row.sheet_music_url) } : {}), ...(row.source_url ? { sourceUrl: String(row.source_url) } : {}) }));
} };
