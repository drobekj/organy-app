...
The technical schema draft adds storage-neutral candidate concepts for later evaluation: Song; MelodyClass / MelodyClassMembership / MelodyLinkEvidence; Person / Account / Actor / RoleAssignment; RepertoireEntry; PreferenceVote; ServiceContext / ServiceSet / ServiceSetRow; CompletedServiceRecord / CompletedServiceRow; AntiphonMapping / LiturgicalSeasonSongMapping; and SourceReference / ImportNote. For Planning Lifecycle First, future architecture and storage evaluation should use only the first-slice subset: minimal Person / Actor / RoleAssignment or equivalent actor-role representation; minimal SongReference by `(language, number)`; ServiceContext; ServiceSet; ServiceSetRow; CompletedServiceRecord; and CompletedServiceRow. These are candidate concepts, not accepted physical tables, SQL, Prisma schema, or implementation tasks. Any first-slice storage decision should be evaluated against `docs/planning-lifecycle-first-schema-subset.md`, `docs/first-slice-storage-decision-preparation.md`, and the accepted single hosted one-congregation deployment assumption. Storage technology remains unresolved.
