...
Future persistence and technical schema design must be derived from `docs/target-domain-persistence-model.md`, `docs/auth-account-role-model.md`, `docs/target-technical-schema-draft.md`, `docs/planning-lifecycle-first-schema-subset.md`, `docs/first-slice-storage-decision-preparation.md`, `docs/first-slice-schema-open-questions-resolution.md`, and `docs/legacy-to-domain-mapping.md`, not copied from the legacy SQL Server schema. The target-domain persistence model identifies the logical data areas SongCatalog, MelodyEquivalence, Repertoire, PreferenceVotes, ServicePlanning, CompletedHistory, People/Roles, and KnowledgeMappings as concepts for future architecture and schema design.

The technical schema draft adds storage-neutral candidate concepts for later evaluation: Song; MelodyClass / MelodyClassMembership / MelodyLinkEvidence; Person / Account / Actor / RoleAssignment; RepertoireEntry; PreferenceVote; ServiceContext / ServiceSet / ServiceSetRow; CompletedServiceRecord / CompletedServiceRow; AntiphonMapping / LiturgicalSeasonSongMapping; and SourceReference / ImportNote. For Planning Lifecycle First, future architecture and storage evaluation should use only the first-slice subset: minimal Person / Actor / RoleAssignment or equivalent actor-role representation; minimal SongReference by `(language, number)`; ServiceContext; ServiceSet; ServiceSetRow; CompletedServiceRecord; and CompletedServiceRow. These are candidate concepts, not accepted physical tables, SQL, Prisma schema, or implementation tasks.
...
The following concerns remain cross-cutting responsibilities. Some are already implemented; their authoritative implementation and operational details live in the referenced code, decisions, and runbooks rather than being duplicated here:
...
- How should any remaining legacy data be assessed, migrated, archived, or retired as the current PostgreSQL domain model continues to replace the legacy boundary?
