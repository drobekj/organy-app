...
3. Resolves the Production backup source without reading back Vercel Sensitive environment variables. If `DATABASE_URL_UNPOOLED` or `DATABASE_URL` is already explicitly present in the operator process, the existing validated path remains available. Otherwise Verify DB invokes pinned Neon CLI through `npx`, uses authenticated structured project discovery, requires exactly one accessible project named `organy-app-production`, and requests that project's default-branch direct connection string. The first Neon CLI use may open a browser authorization flow; Neon CLI stores its own local authentication state, while the database connection string remains process-local.
