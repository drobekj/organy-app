...
- **Authentication infrastructure.** Phases 31.28-31.31 now implement the accepted first identity chain: Better Auth/Drizzle protected username/password Accounts and DB sessions; server-authoritative Account → active Actor → current `app_user_roles`; nickname-only congregation preference access with no protected Account; normal admin future-account/privileged-role administration with last-active-admin protection; own-password change; admin reset of another protected Account password with target-session revocation; and operator-only break-glass reset of an existing protected admin. Still unresolved are production hosting/secrets operations, the concrete real-world credential-delivery procedure, forced-first-change policy, any future public/token-based recovery mechanism, and identity/security telemetry.
...
3. **Authentication/authorization implementation.** The core first-production identity mechanics are now implemented through Phase 31.31: protected username/password staff Accounts, DB sessions, server-derived active Actor/current roles, nickname-only congregation voters, admin protected Account/role administration, own-password change, admin reset of another Account, and operator-only existing-admin credential recovery. Remaining decisions are operational: production secrets, credential handoff procedure, any forced-first-change or future public recovery policy, hosting/cutover, and identity/security telemetry.
...
- Production deployment, secrets, backup/restore, operations hardening, and identity/security telemetry beyond the implemented Phase 31.28-31.31 access mechanics.
...
- Production readiness still depends on deployment, secrets, backup/restore, operational procedures, and broader production test strategy.
...
- [ ] Remaining production identity/access operations completed: concrete secrets management, credential-delivery procedure, production cutover/hosting, and separately accepted security telemetry or stronger recovery policy if required.
