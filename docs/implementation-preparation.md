...
- `docs/production-runtime-runbook.md`
...
- **Database schema.** Coding readiness remains blocked for persistence work. The domain model is conceptual and must not be treated as a target schema; future target schema design now has `docs/target-technical-schema-draft.md` as a storage-neutral draft input alongside `docs/target-domain-persistence-model.md` and `docs/legacy-to-domain-mapping.md`, with Planning Lifecycle First narrowed by `docs/planning-lifecycle-first-schema-subset.md`. Storage decision preparation exists in `docs/first-slice-storage-decision-preparation.md`, `docs/adr-first-slice-storage.md` accepts PostgreSQL-like relational storage at the direction level, and `docs/first-slice-schema-open-questions-resolution.md` resolves key first-slice schema questions at design level. A documentation-only physical schema draft now exists in `docs/first-slice-physical-schema-draft.md`, and `docs/adr-first-slice-tooling.md` selects Drizzle ORM plus `drizzle-kit`, schema location, migration location, config-file location, and reviewable generated SQL as the minimal Phase 6 baseline. This readiness text predates substantial runtime persistence implementation; remaining production-readiness concerns now center on production database/provider and hosting choices, scheduled/off-site backup retention/encryption/PITR operations beyond the Phase 31.33 logical recovery baseline, seed policy, concrete secrets operations beyond the Phase 31.32 preflight, and broader test/operations strategy rather than the already implemented Phase 31.28-31.33 access/runtime/recovery mechanics. Accepting the minimal baseline still does not authorize unrelated new schema, migration, SQL, persistence, or application work outside separately accepted slices.
- **API endpoints.** No general API-contract strategy has been selected beyond the endpoints already introduced by accepted implementation slices.
- **UI components.** Workflows are product-level processes, not a general component specification or screen-design system; existing accepted slice UIs do not settle broader UI architecture.
- **Authentication infrastructure.** Phases 31.28-31.33 now implement the accepted first identity and adjacent recovery chain: Better Auth/Drizzle protected username/password Accounts and DB sessions; server-authoritative Account → active Actor → current `app_user_roles`; nickname-only congregation preference access with no protected Account; normal admin future-account/privileged-role administration with last-active-admin protection; own-password change; admin reset of another protected Account password with target-session revocation; operator-only break-glass reset of an existing protected admin; production runtime/secrets preflight; and whole-database logical recovery with restored protected-session revocation. Still unresolved are hosting/cutover, concrete secret-manager/rotation operations, the real-world credential-delivery procedure, forced-first-change policy, any future public/token-based recovery mechanism, and identity/security telemetry.
...
- **Tests.** Slice-specific acceptance exists, but no final production-wide test strategy, test-layer policy, or operations acceptance model has been selected.
...
2. **Persistence approach.** Storage direction is selected as PostgreSQL-like relational storage for Planning Lifecycle First, and the minimal Phase 6 baseline selects PostgreSQL locally, Drizzle ORM plus `drizzle-kit`, `src/db/schema`, `drizzle`, `drizzle.config.ts`, `DATABASE_URL`, and reviewable generated SQL for the first migration. Phase 31.33 now provides the minimal logical whole-database backup/integrity/separate-target restore rehearsal baseline. Remaining production persistence decisions include the production database provider, connection management beyond local `DATABASE_URL`, scheduled/off-site backup storage and retention, encryption/key management, PITR/WAL and recovery objectives, seed strategy, and broader test/operations strategy, evaluated against the accepted single hosted one-congregation deployment assumption.
3. **Authentication/authorization implementation.** The core first-production identity mechanics and adjacent runtime/recovery baselines are now implemented through Phase 31.33: protected username/password staff Accounts, DB sessions, server-derived active Actor/current roles, nickname-only congregation voters, admin protected Account/role administration, own-password change, admin reset of another Account, operator-only existing-admin credential recovery, production configuration preflight, and restore-time protected-session revocation. Remaining decisions are operational: concrete secret-manager/rotation and credential-handoff procedure, any forced-first-change or future public recovery policy, hosting/cutover, and identity/security telemetry.
...
6. **Identity/security logging boundary.** Phase 31.31 still does not decide whether account/role administration, password reset/recovery, and failed authentication events belong in business audit, security logging, or both.
...
- Production deployment, hosting/cutover, scheduled/off-site backup retention/PITR, broader operations hardening, and identity/security telemetry beyond the implemented Phase 31.28-31.33 access/runtime/logical-recovery baselines.
...
- Production readiness still depends on hosting/cutover, scheduled/off-site backup retention/encryption/PITR, operational procedures, and broader production test strategy.
...
A schema draft, first-slice schema subset, storage decision preparation document, tooling decision preparation document, accepted storage direction, accepted tooling direction, or minimal runnable scaffold baseline does not make the product ready to implement beyond the scaffold. Product implementation should not begin until the following readiness items are satisfied at the appropriate level for the selected first slice.
...
- [ ] Production database/provider, production connection management, hosting/cutover, scheduled/off-site backup retention/encryption/PITR, seed policy, and broader operations strategy completed.
- [x] Minimal logical whole-database backup/integrity/separate-target restore and recovery-rehearsal baseline implemented in Phase 31.33.
...
- [x] Congregation nickname-only own-preference boundary implemented in Phase 31.29 with no protected Account authority.
- [x] Admin future protected Account/privileged-role administration implemented in Phase 31.30 with deactivation/reactivation and last-active-admin safeguard.
- [x] Minimal protected credential reset/recovery boundary implemented in Phase 31.31: admin reset of another protected Account, session revocation, no public recovery flow, and operator-only existing-admin break-glass reset.
- [x] Minimal production runtime/secrets configuration preflight implemented in Phase 31.32.
- [ ] Remaining production identity/access operations completed: concrete secret-manager/rotation, credential-delivery procedure, production cutover/hosting, and separately accepted security telemetry or stronger recovery policy if required.
